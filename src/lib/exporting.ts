...
export async function downloadElementByID(elementID: string): Promise<void> {
	...
	const canvas = await html2canvas(document.body, {
		...
		onclone: (clonedDoc) => {
			...
			clonedElement.querySelectorAll(".unhilighted").forEach((child) => {
				child.classList.remove("unhilighted");
			...
			clonedDoc.querySelectorAll("#tt .item.lesson").forEach((item) => {
				...
				if (styles.outlineStyle !== "none" && parseFloat(styles.outlineWidth) > 0) {
					item.style.boxShadow = `inset 0 0 0 ${styles.outlineWidth} ${styles.outlineColor}`;
				}
			});
		}
	});

	downloadURI(canvas.toDataURL("image/png"), `Tunniplaan ${new Date().toDateString()}.png`);
}
